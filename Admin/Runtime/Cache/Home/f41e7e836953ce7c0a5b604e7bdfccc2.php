...
<body>
	...
	<!-- 创建项目开始 -->
	<form id="add_form" action="<?php echo U('Match/runAdd');?>" method="post" enctype="multipart/form-data">
		<h1>添加一个新的比赛项目</h1>
		<div>
			<label>比赛项目: <input type="text" name="title" id="title" required="required" /></label>
			<label for="">人数限制：<input type="number" name="limit" min="1" max="50" step="1" /></label>
		</div>
		<div>
			<label>比赛说明: <textarea name="info" cols="30" rows="10" style="vertical-align: top;" required="required"></textarea></label>
 ...
 foreach ($types as $key=>$type) { ?>
				<label><input type="checkbox" name="type[]" value="<?=$type['id']?>"><?=$type['type']?></label>&emsp;&emsp;
				<?php
 } ?>
		</fieldset>
		<div id="ext">
