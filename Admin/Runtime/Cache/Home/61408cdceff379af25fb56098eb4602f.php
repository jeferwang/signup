...
<title>后台管理-比赛列表</title>

	<script type="text/javascript">
		//		切换状态
		function toggleStatus($id) {
			$.post("<?php echo U('Match/toggleStatus');?>", {'id': $id}, function () {
		...
		function runDel($id) {
			$.post("<?php echo U('Match/runDel');?>", {'id': $id}, function () {
				location.reload(true);
			});
		}
	</script>
...
<body>
	<div id="top">
		...
		<div class="menu">
			<a href="javascript:void(0)" >当前位置:比赛列表</a>
			
	<a href="<?php echo U('Match/index');?>">比赛列表</a>
	<a href="<?php echo U('Match/add');?>">新增比赛</a>
	...
	<table id="table" border="1" style="width: 97%;margin: 30px 0 0 2%;">
		...
		<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
				...
				<td align="center">
					<a href="javascript:void(0)" onclick="toggleStatus('<?php echo ($vo['id']); ?>')">[发布/结束]</a>
					<a href="<?php echo U('Match/fix',array('id'=>$vo['id']));?>">[修改]</a>
					<a href="javascript:void(0)" onclick="deleteItem('<?php echo ($vo['id']); ?>')">[删除]</a>
				</td>
